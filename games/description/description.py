...
from games.reviews import services as review_services
from games.auth.authentication import login_required
from flask import Blueprint, session, render_template, url_for, redirect, flash
import games.wishlist.services as services
...
def description(game_id):
    ...
    if form.validate_on_submit():
        ...
        new_review = Review(user_id, game, rating, comment)
        repo.repo_instance.add_review(new_review)
        flash('Review submitted successfully!', 'success')

        return redirect(url_for('description_bp.description', game_id=game_id))
    reviews = review_services.get_reviews_for_game(game, repo.repo_instance)
    average_rating = review_services.calculate_average_rating(game, repo.repo_instance)
    user_id = session["username"]
    wishlist_games = services.get_user_wishlist(user_id, repo.repo_instance)
    wishlist_game_ids = [game.game_id for game in wishlist_games]
